    ...
    if (user_option === "Patient") {
      ...
      if (!CheckDuplicate) {
        return res.send({
          success: false,
          status: 200,
          message: "This Email Not Registered So please Register First.",
        });
      } else {
        ...
        return res.send({
          success: true,
          status: 200,
          message: "Email Verification Code Sended Provide Mail address",
        });
      }
    }
...
async function mailingFunction(otp, to) {
  ...
  var mailOptions = {
    ...
    html:
      ...
      otp +
      " is the OTP For Your Reset password ,Valid For 15mins.Please Do not share this with anyone. </p>" +
      "</td>" +
  ...
  const checkStatus = await transporter.sendMail(
    mailOptions,
    function (error, info) {
      return true;
    }
...
const otpVerification = async (req, res, next) => {
  ...
  if (otp_valide) {
    ...
    const timedif = getMinDiff(new Date(inserted_date), new Date(today));
    console.log(timedif);
    if (timedif <= 15) {
...
const passwordUpdate = async (req, res, next) => {
  try {
    ...
    let user_option = req.body.user_type;
    if (user_option === "Patient") {
      let CheckDuplicate = await User.update(
        { password: password },
        {
          where: { email_id: email_id },
        }
      ...
      if (CheckDuplicate) {
        ...
        return res.send({
          success: true,
          status: 200,
          message: "Somthing Went Wrong Password Not Updated",
        });
      }
    }
    //For Docter Forgot password
    else {
      let CheckDuplicate = await Therapist.update(
        { password: password },
        {
          where: { email: email_id },
        }
...
const AddSlats = async (req, res, next) => {
  try {
    ...
    //TO store One by one
    dateArray = await dateArray.map((dateData) => {
      return createSperateDate(therapistId,dateData, slatArray);
    });

    //To Return After Stored 
    return res.send({
      status: true,
...
const createSperateDate = async (therapistId,dateData, timeSlat) => {
  
  //Store Dates One By One
  const InsertedDate = await SlatDate.create({
    date: dateData.slice(0, 10),
    therapist_id: therapistId,
  });

  //To Make Slat Db Insert model for bulk Create
  const slatArray = await timeSlat.map((data) => {
    return {
      slat_time: data.value,
      session: data.Sessionvalue,
      slat_date: InsertedDate.id,
    };
  });

//Store All time slat based on last created date id .
  SlatTiming.bulkCreate(slatArray, { returning: true }).then(function () {
    return true;
  });
};
...
const TherapistProfile = async (req, res, next) => {
  try {
...
const TherapistRegisterProcess = async (req, res, next) => {
  try {
    let first_name = req.body.first_name;
    let last_name = req.body.last_name;
    let email_id = req.body.email_id;
    ...
    let speciallist = req.body.speciallist;

    const CheckDuplicate = await Therapist.findOne({
      where: { email: email_id },
    });
    ...
    if (CheckDuplicate) {
      return res.send({
        success: false,
        status: 200,
        message: "Therapist Email Already Exists",
        data: CheckDuplicate,
      });
    } else {
      ...
      const dataInsert = await Therapist.create({
        ...
        gender:gender,
        specialist_id:speciallist
      });

      if (dataInsert) {
        const getData = await Therapist.findOne({
          where: { email: email_id },
        });
...
try{
  const  collectAll = await Banner.findAll({
    order: [
      ['id', 'DESC'],
       ],
  limit: 10, 
  });
  return res.send({ status: true, message: "Data Collected", data: collectAll });
}
catch(error){
  return res.send({ status: false, message: error.message, data: error });
}
...
const Testimonials = async (req,res,next) => {
  try{
    const  collectAll = await Testimonial.findAll({
      order: [
        ['id', 'DESC'],
         ],
    limit: 15, 
  ...
  const ProfileUpdate = async (req,res,next) =>{
    ...
    let health_issues=req.body.health_issues;
    let mental_health=req.body.mental_health;
    let suicide=req.body.suicide;
    
   //If Profile Not Empty New Profile Upload
    if(req.file)
  {
    ...
    let DataUpdate = await User.update(
      {
        ...
        mental_health_issue_before: mental_health,
        thought_of_suicide: suicide,
    ...
    if(DataUpdate)
    {
      return res.send({ status: true, message: "Profile Updated!" });
    }
    else
    {
  ...
  //If Profile Not Updated Already Profile is Here
  else{
    let DataUpdate = await User.update(
      {
        first_name: first_name,
